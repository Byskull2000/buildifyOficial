  ...
  const PagoConQR: React.FC = () => {
    ...
    useEffect(() => {
      ...
      const fetchQrCode = async () => {
        if (idUsuario) {
          try {
            const response = await fetch(`/api/usuarios/${idUsuario}/qr`);
            if (!response.ok) {
            ...
            if (data && data.imagen_qr) {
              setQrCodeUrl(data.imagen_qr); // Guardar la URL del QR en el estado
            } else {
        ...
        <div className="text-center">
          ...
          {qrCodeUrl === null ? (
            <p className="text-red-500 font-semibold">
              No se puede pagar por QR, el vendedor no tiene un código disponible.
            </p>
          ) : (
            <img
              src={qrCodeUrl}
