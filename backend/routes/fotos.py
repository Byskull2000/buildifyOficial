...
def upload_file():
    ...
    for file in files:
        ...
        try:
            ...
            img.save(img_bytes, format='PNG' if img.format ==
                     'PNG' else 'JPEG')  # Guarda en un formato válido
...
def mostrar_fotos():
    ...
    fotos_data = [{'id': foto.id, 'filename': foto.filename, 'data': b64encode(
        foto.data).decode('utf-8')} for foto in fotos]
