...
def registrar_material():
    try:
        ...
        if not id_tipo_material:
            return (
                jsonify({"message": "El ID del tipo de material es obligatorio"}),
                400,
            )
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "tipo_unidad_material": nuevo_material.tipo_unidad_material,
        ...
        print(e)
        return (
            jsonify({"message": "Error al registrar el material", "error": str(e)}),
            400,
        )
...
def obtener_materiales():
    try:
        ...
        data = [
            {
                ...
                "imagenUrl": (
                    material.imagenes[0].url_imagen if material.imagenes else None
                ),
                "imagenes": [
        ...
        return (
            jsonify({"message": "Materiales obtenidos exitosamente", "data": data}),
            200,
        )

    except Exception as e:
        return (
            jsonify({"message": "Error al obtener los materiales", "error": str(e)}),
            400,
        )
...
def obtener_materiales_por_usuario(id_usuario):
    try:
        ...
        if not materiales:
            return (
                jsonify({"message": "No se encontraron materiales para este usuario"}),
                404,
            )
    ...
    try:
        ...
        if not materiales:
            return (
                jsonify(
                    {
                        "message": "No se encontraron materiales para este tipo de material"
                    }
                ),
                404,
            )
...
def obtener_material_por_id(id_material):
    try:
        ...
        print("inicia")
        resultado = (
            db.session.query(Material, TipoMaterial)
            .join(
                TipoMaterial, Material.id_tipo_material == TipoMaterial.id_tipo_material
            )
            .filter(Material.id_material == id_material)
            .first()
        )
        ...
        return jsonify({"message": "Material obtenido exitosamente", "data": data}), 200

    except Exception as e:
        return (
            jsonify({"message": "Error al obtener el material", "error": str(e)}),
            400,
        )
    ...
    try:
        ...
        if min_precio is None or max_precio is None:
            return (
                jsonify(
                    {
                        "message": "Se deben proporcionar los valores min_precio y max_precio"
                    }
                ),
                400,
            )
        ...
        if not materiales:
            return (
                jsonify(
                    {
                        "message": "No se encontraron materiales en el rango de precios especificado"
                    }
                ),
                404,
            )
    ...
    try:
        ...
        if not materiales:
            return (
                jsonify(
                    {
                        "message": "No se encontraron materiales con el estado especificado"
                    }
                ),
                404,
            )
...
def buscar_materiales():
    try:
        ...
        if not texto_busqueda:
            return (
                jsonify({"message": "Se debe proporcionar el texto de búsqueda"}),
                400,
            )

        # Filtrar materiales que contengan el texto en el nombre
        materiales = Material.query.filter(
            Material.nombre_material.ilike(f"%{texto_busqueda}%")
        ).all()
        ...
        if not materiales:
            return (
                jsonify(
                    {
                        "message": "No se encontraron materiales que coincidan con la búsqueda"
                    }
                ),
                404,
            )
        ...
        return jsonify({"message": "Error al buscar materiales", "error": str(e)}), 400
...
def filtrar_materiales_por_interes():
    try:
        ...
        if id_usuario is None or ciudad is None:
            return (
                jsonify(
                    {"message": "Faltan parámetros obligatorios: id_usuario o ciudad"}
                ),
                400,
            )
        ...
        if id_tipos_material:
            ...
            materiales = (
                Material.query.filter(Material.id_tipo_material.in_(id_tipos_material))
            ...
            materiales = (
                Material.query.filter(
                    Material.descripcion_direccion_material.ilike(f"%{ciudad}%")
                )
                .filter_by(estado_publicacion_material="activo")
                .all()
            )
        ...
        return (
            jsonify(
                {"message": "Error al buscar materiales por intereses", "error": str(e)}
            ),
            400,
        )
    ...
    try:
        ...
        if not materiales:
            return (
                jsonify(
                    {
                        "message": "No se encontraron materiales que coincidan con los filtros"
                    }
                ),
                404,
            )
...
def actualizar_material(id):
    try:
        ...
        return (
            jsonify({"message": "Error al actualizar el material", "error": str(e)}),
            400,
        )
